# I use a hierarchical recurrent neural network implementation, trained on a set of videos with and without accidents, to determine whether a new video contains an accident or not.</br></br></br> 
...
#
#
img_filepath = 'crash_cut' #### the filepath for the training video set
neg = glob.glob(img_filepath + '\\no_crash/*.mp4')               #### negative examples - ACCV
pos = glob.glob(img_filepath + '\\crash1/*.mp4')                 #### positive examples - ACCV
all_files = numpy.concatenate((pos, neg))
print(len(neg), len(pos))                                #### print check
...
labels = numpy.concatenate(([1]*min_len, [0]*min_len))  ### create the labels for the videos
print(labels)
...
x_testA = numpy.asarray(x_t1[len(x_t1)//2:]); y_testA = numpy.asarray(y_t1[len(y_t1)//2:])    #### test set

### valid set for model
x_testB = numpy.asarray(x_t1[:len(x_t1)//2]); y_test = numpy.asarray(y_t1[:len(y_t1)//2])    ### need to be arrays
...
for i in range(epochs):               ### number of epochs
    ...
    x_shuff = numpy.asarray(x_shuff); y_shuff=numpy.asarray(y_shuff) ### back into arrays
...
plt.plot([0,1],[0,1],'k:',alpha=0.5)                       ### plot the "by chance" line - the goal is to achieve better than random accuracy
ys = [y_train, y_test, y_testA]                       ### set up labels to be iterated through
labs = ['Train', 'Validdate', 'Test']                      ### set up tags to be iterated through
...
for i,xset in enumerate([x_train, x_testB, x_testA]):  ### iterate through each set of data
    ...
    plt.plot(fpr, tpr, '-', color=col[i], alpha=0.7, lw=1.5, label=labs[i])      ### plot the ROC curve with false pos rate and true pos rate
...
def pred_model(file_path):
    preds=[]
    d = make_dataset(file_path)                         ### can load all of valid/test datasets at once in memory
    preds = model.predict(d)                    ### predictions with loaded model for each valid/test dataset
    
    fpr, tpr, threshs = sklearn.metrics.roc_curve(file_path[:,1], preds[:,1]) ### get the false pos rate and true pos rate
    #plt.plot(fpr, tpr, '-', color='#4881ea', alpha=0.7, lw=1.5)      ### plot the ROC curve with false pos rate and true pos rate
    
    #print(labs[i])
    print("Area under curve")
    print(sklearn.metrics.auc(fpr, tpr))                ### print area under curve for each set
    print("Accuracy score")
    print(sklearn.metrics.accuracy_score( preds[:,1]))   ### print accuracy for each set
    #print("Confusion matrix")
    #print(sklearn.metrics.confusion_matrix(ys[i][:,1], [round(j) for j in new_pred[:,1]])) ### print confusion matrix for each
...
plt.plot([0,1],[0,1],'k:',alpha=0.5)                  ### plot the "by chance" line - trying so hard to be better than this...
for i,p in enumerate(preds):                      ### for each of the calculated predictions, make a histogram
    plt.hist(p[:,1], bins = numpy.arange(0,1,0.05), histtype='stepfilled', color=col[i], alpha=0.7, label=labs[i])
plt.xlabel('False Positive Rate'); plt.ylabel('True Positive Rate')
